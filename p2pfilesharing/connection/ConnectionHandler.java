  ...
  public void receiveMessage() {
    ...
    new Thread() {
      public void run() {
        ...
        while (true) {
          try {
            ...
            System.out.println("Received message type: " + receivedMsg.getMessageType() + " from: "
                               + neighboringPeer.getPeerId());
            if (receivedMsg != null) {
              if (receivedMsg.getMessageType().equals(MessageType.UNCHOKE)) {
                writeLog.unchokedByNeighbor(neighboringPeer.getPeerId());
                unChokeFlag = true;
                sendRequest();
              } else if (receivedMsg.getMessageType().equals(MessageType.CHOKE)) {
                writeLog.chokedByNeighbor(neighboringPeer.getPeerId());
                unChokeFlag = false;
              } else if (receivedMsg.getMessageType().equals(MessageType.HAVE)) {
                Have have = (Have) (receivedMsg.getMessagePayload());
                FileUtilities.updateBitfield(have.getIndex(), neighboringPeer.getBitField());
                writeLog.receiveHaveMessage(neighboringPeer.getPeerId(), have.getIndex());
              ...
              } else if (receivedMsg.getMessageType().equals(MessageType.REQUEST)) {
                int requestedIndex = ((Request) receivedMsg.getMessagePayload()).getIndex();
                ...
                Message pieceToSend = new Message(MessageType.PIECE,
                                                  new Piece(pieceContent, pieceIndex));
                sendMessage(pieceToSend);
              } else if (receivedMsg.getMessageType().equals(MessageType.INTERESTED)) {
                pManager.add(neighboringPeer);
                writeLog.receiveInterestedMessage(neighboringPeer.getPeerId());
              } else if (receivedMsg.getMessageType().equals(MessageType.NOT_INTERESTED)) {
                pManager.remove(neighboringPeer);
                writeLog.receiveUninterestedMessage(neighboringPeer.getPeerId());
              } else if (receivedMsg.getMessageType().equals(MessageType.BITFIELD)) {
                BitField in_payload = (BitField) (receivedMsg.getMessagePayload());
              ...
              } else if (receivedMsg.getMessageType().equals(MessageType.PIECE)) {
                try {
                  FileManager.store((Piece) receivedMsg.getMessagePayload());
                } catch (Exception e) {
                ...
                pManager.sendHaveAll(((Piece) receivedMsg.getMessagePayload()).getIndex());
                piecesDownloaded++;
                writeLog.finishDownloadingPiece(neighboringPeer.getPeerId(),
                                                ((Piece) receivedMsg.getMessagePayload()).getIndex(),
                                                FileManager.getNoOfPiecesAvailable());
                if (FileManager.getNooffilepieces() == FileManager.getNoOfPiecesAvailable())
                  writeLog.completedDownload();
      ...
      void sendRequest() {
        ...
        Payload requestPayload = new Request(pieceIdx);
